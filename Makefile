# This Makefile is for the MD5Check extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/MD5Check.pm]
...
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/MD5Check.pm]
#     clean => { FILES=>q[MD5Check-*] }
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) -u \
	  lib/MD5Check.pm $(INST_MAN3DIR)/MD5Check.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: "Use it for init Web files'\''s md5 values of your site(or other dir), and check if it changed!"' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''orange <linzhe@ijz.me>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: MD5Check' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "Use it for init Web files'\''s md5 values of your site(or other dir), and check if it changed!",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "orange <linzhe@ijz.me>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "MD5Check",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="MD5Check" VERSION="0.06">' > MD5Check.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Use it for init Web files'\''s md5 values of your site(or other dir), and check if it changed!</ABSTRACT>' >> MD5Check.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>orange &lt;linzhe@ijz.me&gt;</AUTHOR>' >> MD5Check.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> MD5Check.ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> MD5Check.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-5.18" />' >> MD5Check.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> MD5Check.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> MD5Check.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> MD5Check.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/MD5Check.pm' 'blib/lib/MD5Check.pm' 
